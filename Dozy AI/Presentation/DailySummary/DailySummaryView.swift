...
struct DailySummaryView: View {
    ...
    init(
        ...
        let vm = DailySummaryViewModel(container: container)
        vm.events = events
...
private extension DailySummaryView {
    ...
    // 시간대 활동 차트
    // 막대 차트로 시간대별 활동 밀도를 보여주면
    // "오전에 회의가 몰려있네" 같은 인사이트를 얻을 수 있습니다.
                    ...
                    VStack(spacing: 2) {
                        ...
                        VStack(spacing: 0) {
                            Rectangle()
                                ...
                                .frame(
                                    height: CGFloat(activity.taskCount) / CGFloat(maxCount) * 80
                                )
                            Rectangle()
                                .fill(Color.blue.opacity(0.7))
                                .frame(
                                    height: CGFloat(activity.eventCount) / CGFloat(maxCount) * 80
                                )
                        }
...
private extension DailySummaryView {
    ...
    var weeklyAverageCard: some View {
        HStack(spacing: 20) {
            ...
            VStack {
                let totalEvents = viewModel.weeklyTrend.reduce(0) { $0 + $1.eventCount }
                Text("\(totalEvents)")
                    .font(.system(size: 28, weight: .bold, design: .rounded))
                Text("총 일정")
                    .font(.caption2)
            ...
            VStack {
                let totalTasks = viewModel.weeklyTrend.reduce(0) { $0 + $1.taskCount }
                Text("\(totalTasks)")
                    .font(.system(size: 28, weight: .bold, design: .rounded))
                Text("총 완료")
                    .font(.caption2)
                ...
                ForEach(viewModel.weeklyTrend) { point in
                    VStack(spacing: 4) {
                        ...
                        RoundedRectangle(cornerRadius: 4)
                            ...
                            .frame(
                                height: max(point.score / maxScore * 100, 4)
                            )
    ...
    var weeklyActivityChart: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            let maxActivity = max(
                viewModel.weeklyTrend.map { $0.eventCount + $0.taskCount }.max() ?? 1,
                1
            )
                    ...
                    VStack(spacing: 4) {
                        VStack(spacing: 0) {
                            Rectangle()
                                .fill(Color.green.opacity(0.7))
                                .frame(
                                    height: CGFloat(point.taskCount) / CGFloat(maxActivity) * 80
                                )
                            Rectangle()
                                .fill(Color.blue.opacity(0.7))
                                .frame(
                                    height: CGFloat(point.eventCount) / CGFloat(maxActivity) * 80
                                )
                        }
                        .frame(height: 80, alignment: .bottom)
            ...
            if !viewModel.categoryDistribution.isEmpty {
                ...
                ForEach(viewModel.categoryDistribution) { dist in
                    HStack(spacing: 8) {
                        ...
                        Text("\(dist.category.emoji) \(dist.category.rawValue)")
                            .font(.caption)
...
// MARK: - Generating / Empty / Error (공통)
private extension DailySummaryView {
